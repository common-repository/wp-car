...
Plugin Name: WP Car
Version: 1.1
Plugin URI: http://www.ism-soft.com/tool/wp-car-plugin
Description: WP Car shows a car photo whith horsepower( HP ) description on Your sidebar.
Author: ISM
...
<style>
 #wp-car{  }
 #wp_car_image_in{padding: 0px 0px 0px 0px; width: 100%; text-align: center; height:220px;}
 #img_wp_car{ display: block; margin: 2px auto;
		width: 190px; height: 173px; 
		background: url(\''.get_option('home')  .'/wp-content/plugins/wp-car/images/bg.png\');}
 #url_wp_car{width: 80px; margin: 1px 10px 1px 125px; display: block; text-align: left; height: 10px;}
 .wp_car_img{ width: 150px; height:136px; margin: 20px 12px; border: 0px;}
 #desc_wp_car{ margin: 0px auto; line-height: 12px; width: 170px; padding: 0px 10px;}
 #wp_car_kcal_txt{color:#282828;
font-size:40px;
line-height:40px;
font-weight:bold;
margin:-55px 20px 0px 0;
text-align:right;	
position:relative;
height:10px;}
#wp_car_kcal_txt_s{ font-size: 12px; }
	...
	$wp_car_end ="
		...
		<div id='url_wp_car' style='font-size: 8px;  line-height: 10px; height: 10px;'>
			WP Car by <a style='font-size: 8px;' href='http://www.sigma1.pl' title='WP Car Plugin sponsored by OSK Sigma' >Sigma</a> 
		</div>
...
$wp_car = array(
	...
	$output_fin = '<ul class="wp_car">' . $wp_car_start . '<a rel="nofallow" title="'.$output["name"].'" href="'. $output["url"] .'"><img class="wp_car_img" src="'. get_option('home')  .'/wp-content/plugins/wp-car/images/' 
	...
	. '</div><div id="desc_wp_car">' . $output["name"] .' - '. $output["hp"] .' brake horsepower' .', '. $output["tt100"] .' sec. from  0–100 km/h ' . $wp_car_end . '<div style="clear: both; height:1px;"></div></ul>';
...
function widget_sidebar_wp_car() {
	...
	register_sidebar_widget('WP Car', 'widget_wp_car');
}
